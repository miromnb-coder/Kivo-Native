...
export function KivoChatScreenStreaming() {
  ...
  function animateSidebar(toValue: number) {
    ...
    Animated.timing(sidebarProgress, {
      ...
      if (toValue === 0) setSidebarVisible(false);
    });
  }
  ...
  useEffect(() => {
    ...
    return () => sidebarProgress.removeListener(listener);
  }, [sidebarProgress]);
  ...
  const screenPanResponder = useMemo(
    () => PanResponder.create({
      ...
      onPanResponderMove: (_, gesture) => {
        const next = clamp(sidebarProgressRef.current + gesture.dx / pushedDistance);
        setSidebarGestureProgress(next);
      },
      onPanResponderRelease: () => {
        endSidebarGesture(sidebarProgressRef.current > 0.42);
      },
      onPanResponderTerminate: () => {
        endSidebarGesture(sidebarProgressRef.current > 0.5);
      },
    }),
    [pushedDistance, sidebarVisible],
  );
